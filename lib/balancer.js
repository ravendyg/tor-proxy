...
const availableMirrors = ['meself', 'http://83.220.174.108:3014/'];

module.exports = function isRedirectRequired(req, res, next) {
  if (!req.headers.redirected) {
    const
      // mirror = Math.round(Math.random() * availableMirrors.length),
      baseUrl = availableMirrors[1] // mirror]
      ;
    if (baseUrl !== 'meself') {
      const
        url = baseUrl + '?echo=' + req.query.url;
